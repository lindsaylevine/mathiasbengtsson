...
export const Img = ({ src, alt = "", width, height, lazy = false }) => {
  ...
  const path = `${src}?fm=jpg&fl=progressive&w=${width < MAX_WIDTH ? width : MAX_WIDTH}&h=${width < MAX_WIDTH ? height : Math.floor((height / width) * MAX_WIDTH)}`
